...
public enum KeyboardShortcuts {
	/**
	The authoritative result of the latest Carbon registration attempt for a shortcut.

	A stored shortcut can still have a failed registration result. This distinction
	is required because persistence and `RegisterEventHotKey` are separate operations.
	*/
	...
	private static var registrationResults = [Shortcut: RegistrationResult]()
	private static var registrationOwners = [Shortcut: Set<Name>]()
	...
	private static var namesForLegacyHandlers: Set<Name> {
		Set([legacyKeyDownHandlers.keys, legacyKeyUpHandlers.keys].flatMap { $0 })
	}

	private static var namesForStreamHandlers: Set<Name> {
		Set([streamKeyDownHandlers.keys, streamKeyUpHandlers.keys].flatMap { $0 })
	}

	private static var namesForHandlers: Set<Name> {
		namesForLegacyHandlers.union(namesForStreamHandlers)
	}
	...
	*/
	...
	@discardableResult
	private static func register(_ shortcut: Shortcut, for name: Name) -> RegistrationResult {
		guard !registeredShortcuts.contains(shortcut) else {
			registrationOwners[shortcut, default: []].insert(name)
			registrationResults[shortcut] = .registered
			return .registered
		}

		let status = CarbonKeyboardShortcuts.register(
			shortcut,
			onKeyDown: handleOnKeyDown,
			onKeyUp: handleOnKeyUp
		)

		let result = synchronizeRegistrationResult(for: shortcut, status: status)
	...
	@discardableResult
	static func synchronizeRegistrationResult(
		...
		registrationResults[shortcut] = result
		return result
	}

	static func isDispatchEligible(_ shortcut: Shortcut) -> Bool {
		isEnabled
			&& registeredShortcuts.contains(shortcut)
			&& registrationResults[shortcut] == .registered
			&& registrationOwners[shortcut]?.isEmpty == false
	}
	...
	*/
	private static func registerShortcutIfNeeded(for name: Name) {
		...
		register(shortcut, for: name)
	}

	private static func unregister(_ shortcut: Shortcut, for name: Name) {
		guard var owners = registrationOwners[shortcut] else {
			if !registeredShortcuts.contains(shortcut) {
				registrationResults.removeValue(forKey: shortcut)
			}
			return
		}
		...
		guard owners.isEmpty else {
			registrationOwners[shortcut] = owners
			return
		}

		registrationOwners[shortcut] = nil
	...
	*/
	private static func unregisterShortcutIfNeeded(for name: Name) {
		guard
			!namesForHandlers.contains(name),
			let shortcut = name.shortcut
		else {
			return
		}

		unregister(shortcut, for: name)
	}

	private static func unregisterAll() {
		CarbonKeyboardShortcuts.unregisterAll()
		registeredShortcuts.removeAll()
		registrationResults.removeAll()
		registrationOwners.removeAll()
	...
	*/
	public static func removeAllHandlers() {
		let namesToUnregister = namesForLegacyHandlers.subtracting(namesForStreamHandlers)

		for name in namesToUnregister {
			guard let shortcut = getShortcut(for: name) else {
				continue
			}
	...
	*/
	public static func removeHandler(for name: Name) {
		...
		guard
			...
			!namesForStreamHandlers.contains(name)
		else {
			return
		}
	...
	*/
	public static func isEnabled(for name: Name) -> Bool {
		...
		return registeredShortcuts.contains(shortcut)
			&& registrationOwners[shortcut]?.contains(name) == true
	}

	/**
	Returns the authoritative result from the latest underlying
	`RegisterEventHotKey` attempt for the shortcut currently stored for `name`.
	*/
	public static func registrationResult(for name: Name) -> RegistrationResult? {
		guard let shortcut = getShortcut(for: name) else {
			return nil
		}

		return registrationResults[shortcut]
	}
	...
	*/
	...
	static func userDefaultsSet(name: Name, shortcut: Shortcut) {
		...
		if let oldShortcut = getShortcut(for: name) {
			unregister(oldShortcut, for: name)
		}
